...
async def create_mcp_agent_with_history():
    ...
    async with sse_client(mcp_url) as (read_stream, write_stream):
        async with ClientSession(read_stream, write_stream) as mcp_session:
            ...
            print(f"✓ Discovered {len(mcp_tools.tools)} tools: {[t.name for t in mcp_tools.tools]}\n")
    ...
    If a user refers to "that station" or "the same line", use context from conversation history."""
            ...
            if os.getenv("LANGCHAIN_TRACING_V2") == "true":
                ...
                print("  • Tool invocations (get_train_schedule) with arguments")
                print("  • Conversation flow and decision points")
