import { API_PROVIDERS, getProviderEndpoint, getAuthHeader } from '../apiProviders';
...
describe('API Providers', () => {
  describe('API_PROVIDERS', () => {
    ...
    test('Grok provider should have correct configuration', () => {
      ...
      expect(grok.defaultModel).toBe('grok-3-mini');
    });

    test('Custom provider should have correct configuration', () => {
      const custom = API_PROVIDERS.custom;
      expect(custom.id).toBe('custom');
      expect(custom.name).toBe('Custom API');
      expect(custom.baseUrl).toBe('');
      expect(custom.requiresAuth).toBe(false);
      expect(custom.supportsStreaming).toBe(true);
    });
  });
  ...
  describe('getProviderEndpoint', () => {
    ...
    test('should return empty string for unknown provider', () => {
      const endpoint = getProviderEndpoint('unknown' as any);
      expect(endpoint).toBe('');
    });
  });
